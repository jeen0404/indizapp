...
class CrushListforotheruser extends StatefulWidget {
  var otheruserid;
  CrushListforotheruser(id){
    this.otheruserid=id;
  }
  @override
  _CrushListListState createState() => _CrushListListState(otheruserid);
}

class _CrushListListState extends State<CrushListforotheruser> with AutomaticKeepAliveClientMixin{
  var otheruserid;
  _CrushListListState(id){
    this.otheruserid=id;
  }
  ...
  Future<void> getfollowinglist() async{
    ...
    await Firestore.instance.collection("users").document(otheruserid).collection("crush").orderBy("id").limit(_perpage).getDocuments()
  ...
  getmorefollowinglist() async {
    ...
    if (getingmoreproduct == false) {
      ...
      await Firestore.instance.collection("users").document(otheruserid).collection("crush").orderBy("id").startAfter([_lastnode.data["id"]]).limit(
          _perpage)
          .getDocuments()
          .
      then((QuerySnapshot qn) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar:AppBar(
        ...
        title: Text("Crush",style: TextStyle(color: Colors.blueAccent),),
        actions: <Widget>[
          IconButton(icon: Icon(Icons.info_outline), onPressed: (){
            return showDialog(barrierDismissible: true,context: context,builder: (BuildContext contaxt){
              return   AlertDialog(content: Container(
                height:120.0,
                child:Center(child: Text("If you like someone and your heart melts for him or her then its definitely a crush. This feature enables you to add that person in crush list. This is list is visible to you and followers only, not to public.",
