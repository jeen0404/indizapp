...
class FollowerlistForOtherUser extends StatefulWidget {
  var otheruserid;
  FollowerlistForOtherUser(id){
    this.otheruserid=id;
  }
  @override
  _FollowerListState createState() => _FollowerListState(this.otheruserid);
}

class _FollowerListState extends State<FollowerlistForOtherUser> with AutomaticKeepAliveClientMixin {
  var otheruserid;
  _FollowerListState(id){
    this.otheruserid=id;
  }
  FirebaseUser user;
  ...
  Future<void> getfollowinglist() async{
    ...
    await Firestore.instance.collection("users").document(otheruserid).collection("follower").orderBy("id").limit(_perpage).getDocuments()
  ...
  getmorefollowinglist() async {
    ...
    if (getingmoreproduct == false) {
      ...
      await Firestore.instance.collection("users").document(otheruserid).collection("follower").orderBy("id").startAfter([_lastnode.data["id"]]).limit(
          _perpage)
          .getDocuments()
          .
      then((QuerySnapshot qn) {
