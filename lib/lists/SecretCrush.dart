...
import 'SecretCrushListCard.dart';

class SecretCrushList extends StatefulWidget {
  @override
  _SecretCrushListState createState() => _SecretCrushListState();
}

class _SecretCrushListState extends State<SecretCrushList> with AutomaticKeepAliveClientMixin{
  FirebaseUser user;
  ...
  Future<void> getfollowinglist() async{
    ...
    await Firestore.instance.collection("users").document(user.uid).collection("secret_crush").orderBy("id").limit(_perpage).getDocuments()
  ...
  getmorefollowinglist() async {
    ...
    if (getingmoreproduct == false) {
      ...
      await Firestore.instance.collection("users").document(user.uid).collection("secret_crush").orderBy("id").startAfter([_lastnode.data["id"]]).limit(
          _perpage)
          .getDocuments()
          .
      then((QuerySnapshot qn) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar:AppBar(
        ...
        title: Text("Secret crush",style: TextStyle(color: Colors.black),),
        actions: <Widget>[
          IconButton(icon: Icon(Icons.info_outline), onPressed: (){
            return showDialog(barrierDismissible: true,context: context,builder: (BuildContext contaxt){
              return   AlertDialog(content: Container(
                height:170.0,
                child:Center(child: Text("Visible ONLY TO YOU and YOUR SECRET CRUSH. List of users that you've shown secret crush on are listed here. Say, if you have secret crush on someone and that person too shows secret crush on you - Then it's a Match listed below with Heart icon - Yes, Congrats!",
      ...
      LiquidPullToRefresh(
        //key: _refreshIndicatorKey,	// key if you want to add
          ...
          child:ListView.builder(itemBuilder:(_,index){
            ...
            SecretCrushListCard(_lists[index]);
