...
import 'package:flutter_app/lists/listcardforcrushonyou.dart';
import 'package:liquid_pull_to_refresh/liquid_pull_to_refresh.dart';

class CrushOnYou extends StatefulWidget {
  @override
  _CrushOnYouState createState() => _CrushOnYouState();
}

class _CrushOnYouState extends State<CrushOnYou> with AutomaticKeepAliveClientMixin {
  FirebaseUser user;
  ...
  Future<void> getfollowinglist() async{
    ...
    await Firestore.instance.collection("users").document(user.uid).collection("crush_on_you").orderBy("id").limit(_perpage).getDocuments()
  ...
  getmorefollowinglist() async {
    ...
    if (getingmoreproduct == false) {
      ...
      await Firestore.instance.collection("users").document(user.uid).collection("crush_on_you").orderBy("id").startAfter([_lastnode.data["id"]]).limit(
          _perpage)
          .getDocuments()
          .
      then((QuerySnapshot qn) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar:AppBar(
        ...
        title: Text("Crush on you",style: TextStyle(color: Colors.black),),
        actions: <Widget>[
      ...
      body:loading?Container(child: Center(child: CircularProgressIndicator(),),):LiquidPullToRefresh(
        //key: _refreshIndicatorKey,	// key if you want to add
          ...
          child:ListView.builder(itemBuilder:(_,index){
            ...
            list_card_for_Crush_on_you(_lists[index].data["id"]);
