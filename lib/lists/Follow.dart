...
class _FollowingListState extends State<FollowingList> with AutomaticKeepAliveClientMixin {
  ...
  Future<void> getfollowinglist() async{
    ...
    await Firestore.instance.collection("users").document(user.uid).collection("follow").orderBy("id").limit(_perpage).getDocuments()
  ...
  getmorefollowinglist() async {
    ...
    if (getingmoreproduct == false) {
      ...
      await Firestore.instance.collection("users").document(user.uid).collection("follow").orderBy("id").startAfter([_lastnode.data["id"]]).limit(
          _perpage)
          .getDocuments()
          .
      then((QuerySnapshot qn) {
